\section{Theoretisch kader}
Voordat er ontworpen kan worden zal er eerst onderzoek gedaan moeten worden naar de werking van een ISFET. Dit is nodig om te weten hoe de pH-waarde uitgelezen kan worden.
Hiernaast zal er, omdat de module zichzelf ook moet kunnen opladen, ook onderzoek gedaan moeten worden naar energy harvesting.
Dit hoofdstuk zal ingaan op de bevindingen van het onderzoeken van beide onderwerpen.

\subsection{De pH-waarde}\label{sec:ph}
\input{sections/ph}

\subsection{De werking van een ISFET}\label[chapter]{sec:werkingISFET}
\input{sections/werkingISFET}

% \subsection{Empirisch onderzoek}
% \subsection{Gebruikersonderzoek}
% \subsection{Methodes}


\input{sections/powerAndHarvest.tex}
