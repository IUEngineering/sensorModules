...
\end{equation}
In het ontwerpen van communicatie systemen komt het vaker voor dat men wil weten wat de verzwakking van een signaal is in decibel. De verzwakking van een signaal kan berekend worden door \autoref{eq:receivedPowerFreeSpace} te herschrijven naar \autoref{eq:FreeSpacePathLoss}.
\begin{equation}\label{eq:FreeSpacePathLoss}
    PL_{FS}=20\log\left(\frac{4\pi d}{\lambda\sqrt{G_tG_r}}\right) \,\,\left[\unit{\decibel}\right]
\end{equation}
In het geval dat er antennes worden gebruikt met isotrope eigenschappen volgt \autoref{eq:isotropicPathLoss} uit \autoref{eq:FreeSpacePathLoss}. Dit is het geval omdat een antenne met isotrope eigenschappen een antenne gain van $1$ heeft \cite{bensky2019shortRangeWirelessCommunication}.
\begin{equation} \label{eq:isotropicPathLoss}
    PL_{FS}=20\log\left(\frac{4\pi d}{\lambda}\right) \,\,\left[\unit{\decibel}\right]
\end{equation}

Het path loss model dat beschreven wordt met \autoref{eq:FreeSpacePathLoss} gaat er van uit dat de zend- en ontvangende antennes zich in free space bevinden. Dit is echter niet het geval en het model van \autoref{eq:FreeSpacePathLoss} zal aangepast moeten worden om meer niet ideale effecten mee te nemen. Een niet idealiteit dat meegenomen kan worden is dat de antennes boven een vlak worden geplaatst. Via het vlak waar de antennes boven staan zal er een reflectie ontstaan. Dit is schematisch getoond in \autoref{fig:twoRayModelSchem}. Dit nieuwe model staat bekend in de literatuur als het two ray model \cite{MobileAntenaSystemsHandbookCH2}.
\begin{figure}[h]
    \centering
...
De voorgaande modellen zijn niet compleet genoeg om de situatie in een bebouwd gebied te voorspellen. Om een beter model te krijgen kan er een fittings factor worden toegevoegd aan \autoref{eq:FreeSpacePathLoss} \cite[24]{brini2019system}. \autoref{eq:fittingFactor} toont deze fittings factor waarin \(l_f\) een fittings coefficient is en \autoref{eq:calcPathLoss} toont de resulterende vergelijking voor het berekenen van de verwachte path loss.
